...
import { employeeCreate, employeesByManager, employeesUPdate, viewEmployeesProfile } from "../../redux/slice/employee/employeeCreateSlice";
...
const EmployeeEdit = ({ hideCompanyLayout = false, role = "", department = "" }) => {
    ...
    const { employeeProfile: employee, error } = useSelector(
        (state) => state.reducer.employee
    );

    useEffect(() => {
        dispatch(viewEmployeesProfile(employeeId));
        // viewEmployeesProfile
    }, [dispatch, employeeId]);
    useEffect(() => {
        if (employee) {
    ...
    const content = (
            ...
            <input type="date" name="dob" value={formData.dob} onChange={handleChange} className={`input ${errors.dob ? "error shake" : ""}`} />
                    ...
                    <Field label="Date Of Birth" icon={<Calendar size={16} />}>
                        <input
                            ...
                            value={
                                formData.dob
                                    ? new Date(formData.dob).toISOString().split("T")[0] // converts to YYYY-MM-DD
                                    : ""
                            }
                            onChange={handleChange}
                            className={`input ${errors.dob ? "error shake" : ""}`}
                        />
                    </Field>


                    <Field label="Profile Picture" icon={<Image size={16} />}>
                        {(formData?.profilePic || employee?.profilePic?.url) && (
                            <img
                                src={
                                    formData.profilePic instanceof File
                                        ? URL.createObjectURL(formData.profilePic) // new upload preview
                                        : formData.profilePic?.url || employee.profilePic?.url // existing image
                                }
                                alt="Profile Preview"
                        ...
                        <input
                            ...
                            name="profilePic"
                            onChange={handleChange}
                            className={`input ${errors.profilePic ? "error shake" : ""}`}
                        />
                    </Field>
            ...
            {currentStep === 1 && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="relative">
                        <label className="text-sm font-medium text-gray-600 mb-1 flex items-center gap-1">
                            Reporting Manager-{formData?.reportingManager?.name}
                        </label>
                ...
                <div className="space-y-4">
                    ...
                    <Field label="Aadhar">
                        {formData?.aadhar?.length !== 0 && (
                            <div className="mt-2">
                                <p className="text-sm font-medium">Existing Aadhaar:</p>

                                <div className="flex gap-3 flex-wrap">
                                    {formData.aadhar.map((url, i) => (
                                        <img
                                            key={i}
                                            src={url.url}
                                            alt={`Aadhaar ${i + 1}`}
                    ...
                    <Field label="PAN">
                        {formData?.pan?.length > 0 && (
                            <div className="mt-2">
                                <p className="text-sm font-medium">Existing PAN:</p>

                                <div className="flex gap-3 flex-wrap">
                                    {formData.pan.map((url, i) => (
                                        <img
                                            key={i}
                                            src={url.url}
                                            alt={`PAN ${i + 1}`}
        ...
        ${submitStart ? "bg-gray-400 cursor-not-allowed" : "bg-green-600 hover:bg-green-700"}`}
                    >
                        {submitStart ? "Updating..." : "Update Employee"}
                    </button>
                )}
            </div>
